...
short int encode(char* inst){ //instructions must be encoded (as bin,dec or String) before it can be used
...
char *intToBinaryString(int num) { //This is AI, i didnt check its correctness
  if (num == 0) {
...
short int binaryStringToShort(const char *binaryString) { //Also AI, also didnt check its correctness
  ...
  while (binaryString[i] != '\0') {
      if (binaryString[i] == '0' || binaryString[i] == '1') {
          ...
          if (sign == 1 && result > 32767) { // 2^15 - 1
              return 32767;
          } else if (sign == -1 && result > 32768) { // 2^15
              return -32768;
          }
...
void execute (int instruction){ //Didnt implement SREG Flags!!!
  int opcode;
  int R1;
  int R2;
  int Imm;
